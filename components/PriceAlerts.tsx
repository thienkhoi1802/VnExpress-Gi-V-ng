...
import { Bell, BellPlus, X, Trash2, TrendingUp, TrendingDown, AlertCircle, CheckCircle2 } from 'lucide-react';
...
export const PriceAlerts: React.FC<PriceAlertsProps> = ({ products, alerts, onAddAlert, onRemoveAlert, hideHeader = true }) => {
    ...
    <div className="bg-white font-sans overflow-hidden">
      {!hideHeader && (
        <div className="p-3 sm:p-4 border-b border-gray-100 flex items-center justify-between">
          <div className="flex items-center gap-2">
             ...
             <h2 className="text-lg font-serif font-bold text-gray-900">Thông báo giá</h2>
          </div>
        </div>
      )}
      ...
      <div className="p-4">
        {!isAdding && (
           ...
           >
             <BellPlus size={16} /> <span>Thêm ngưỡng thông báo mới</span>
           </button>
        )}
        ...
        {isAdding && (
          <div className="mb-6 p-4 border border-vne-red/20 bg-[#fffcfd] relative animate-in fade-in slide-in-from-top-2">
            <button onClick={() => setIsAdding(false)} className="absolute top-2 right-2 p-1 text-gray-400 hover:text-gray-600">
              <X size={16} />
            </button>
            <h3 className="text-[12px] font-bold text-vne-red mb-3 uppercase tracking-wide">Thiết lập ngưỡng giá</h3>
              ...
              <div>
                ...
                <div className="relative">
                  <input 
                    ...
                    className="w-full h-10 px-3 border border-gray-300 text-sm font-bold text-gray-900 outline-none focus:border-vne-red"
                  />
                  <div className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] font-bold text-gray-400 uppercase pointer-events-none">
                    {unitLabel}
                  </div>
                </div>
              </div>
            ...
            <div className="mt-4 flex justify-end gap-2">
               <button onClick={() => setIsAdding(false)} className="px-4 py-1.5 text-[12px] font-bold text-gray-500">Hủy</button>
               <button onClick={handleAdd} className="px-6 py-1.5 bg-vne-red text-white text-[12px] font-bold shadow-sm active:scale-95">Thêm</button>
            </div>
          </div>
        )}
        ...
        <div className="space-y-2">
          {alerts.length === 0 ? (
            <div className="py-6 sm:py-10 flex flex-col items-center justify-center text-gray-400">
                ...
                <p className="text-[14px] sm:text-[17px] font-bold text-gray-500 text-center px-4">Chưa có thông báo nào được thiết lập</p>
            ...
            alerts.map(alert => (
              <div key={alert.id} className="flex items-center justify-between p-3 border border-gray-100 hover:border-gray-200 transition-colors group bg-white shadow-sm">
                <div className="flex items-center gap-3">
                  <div className={`p-2 rounded-full ${alert.type === 'above' ? 'bg-green-50 text-vne-green' : 'bg-red-50 text-[#bd0000]'}`}>
                    {alert.type === 'above' ? <TrendingUp size={16} /> : <TrendingDown size={16} />}
                  </div>
                  <div>
                    <div className="flex items-center gap-2">
                       <span className="text-[13px] font-bold text-gray-900">{alert.productName}</span>
                       <span className="text-[10px] px-1.5 py-0.5 bg-gray-100 text-gray-500 font-bold uppercase rounded-sm">
                         {alert.priceType === 'sell' ? 'Bán ra' : 'Mua vào'}
                       </span>
                    </div>
                    <div className="flex items-center gap-1.5 mt-0.5">
                       <span className="text-[12px] text-gray-400">{alert.type === 'above' ? 'Tăng trên' : 'Giảm dưới'}</span>
                       <span className="text-[15px] font-black text-gray-900 tabular-nums">{alert.targetPrice.toLocaleString()}</span>
                    </div>
                  </div>
                </div>
      ...
      {alerts.length > 0 && (
        <div className="px-4 py-2.5 bg-gray-50 border-t border-gray-100 flex items-center gap-2">
           ...
           <span className="text-[10px] font-bold text-gray-400 uppercase tracking-tight">Hệ thống đang theo dõi giá trực tuyến</span>
        </div>
      )}
    </div>
  );
